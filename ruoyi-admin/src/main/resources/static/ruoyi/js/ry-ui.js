    ...
    $.extend({
    	...
    	table: {
            ...
            initEvent: function(data) {
            	...
            	$("#" + $.table._option.id).on("check.bs.table uncheck.bs.table check-all.bs.table uncheck-all.bs.table load-success.bs.table", function () {
            		...
            		$('#' + $.table._option.toolbar + ' .btn-apply').toggleClass('disabled', rows.length!=1);
        ...
        operate: {
			...
			apply: function() {
					...
					var url = $.table._option.applyUrl.replace("{id}", row[$.table._option.uniqueId]);
					$.modal.open("申请" + $.table._option.modalName, url);
				}
        },
        // 校验封装处理
        validate: {
